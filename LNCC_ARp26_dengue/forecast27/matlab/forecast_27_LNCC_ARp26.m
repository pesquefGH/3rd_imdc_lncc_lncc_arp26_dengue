% Forecast 27: LNCC ARP26 forecast model (high-order AR(p) model)
...
M1 = readtable(['IMDC2026_AggregatedData_',UF,'.csv']); 
% reads data related to the selected state

M2 = readtable(['IMDC2026_AggregatedData_',UF,'_updated_2026.csv']); 
...
ind_f27=649+52+52+52+52;  % index of the end of EW 25 of 2026 
% (upper bound observed data for forecast task 2027)

cc=kcc(1:ind_f27);  % observed cases from EW 1 of 2010 to EW 25 of 2025

cc(cc==0)=0.1;  % replaces null value in cc with a small value. This is 
% because we will apply 
% a log2 function to the data values. 

lwea=2*52-25;   % number of EW to be forecast, i.e., from EW 26 of 2026 
% up to EW 52 2027 (will be cropped later to EW 41/2026 to EW 40/2027)
...
p=92; % model order (p is chosen arbitrarily and should be larger than 52
% (season period))
a=arburg(sig,p);  % AR model estimation via the Burg's method (stable)
a=a(:); % makes it a column vector

e1=filter(a,1,sig);  % obtains the modeling error via inverse filtering
% Obs.: e1 will be used to obtain the initial conditions (IC) of 
% the direct filter, during forecast
...
% excitation signals are all synchronized from EW 1 to EW 52 of the
% subsequent year. We later trim-off the first 25 rows, so that the 
% excitation signals cover EW 26 of a given year to EW 52 of the
...
mE=mean(E);  % row vector of the mean excitation, i.e., mean of the 
% ensemble of excitation signals of PP samples
stdE=std(E); % row vector of the standard deviation of the 
% ensemble of excitation signals of PP samples

med_stdE=median(stdE); % median of vector stdE (informs the 
% standard deviation of 
% a typical white gaussian noise to be generated to excite 
% the direct AP(p) model

% Note: we could use a different standard deviation for each EW, as they 
% are not about equal  
...
stdE2=med_stdE.*ones(size(stdE)); % row vector (PP values) with the 
% same value med_stdE

eek=mE(26:end);  % known mean excitation related to
% EW 26 of 2010 to EW 25 of 2026 
eek=eek(:);  % make it a column vector

[aux,zf]=filter(1,a,e1);  % obtains the initial conditions 'zf' 
% of the direct source-model filter
...
% Now, we generate the set of realizations of artificial model excitation signals.
RE=randn(MC,PP);  % matrix with random realizations of white Gaussian 
% noise (zero-mean), unit variance 
RE=RE.*(1.0*stdE2); % adjust variance of the noise according to that 
% of the mean excitation 
% We keep a zero-mean realizations and only adjust the variance.

RE(:,1:25)=[];  % removes first 25 columns (since forecast starts from 
% EW 26 onward)
...
% Obs. We found out experimentally that it is better to lowpass filtering log2 of the
% percentile lower- and upper bounds then revert back to the original scale 

lower_95 = log2(PP(1,:));  % 2.5% percentile in log 2 scale
...
upper_95 = log2(PP(9,:)); % 97.5% percentile in log 2 scale


% Note: the forecast results (mean, upper and lower bounds in log 2 scale) are then 
% filtered by an SSA (Singular Spectral Analysis) reconstruction filter
% and reverted back to the original scale.
...
indf_ini=665+52+52+52+52; % time index of the EW 41 2026
indf_end=716+52+52+52+52+1; % time index of the EW 41 2027  
...
writetable(T,['..\spreadsheets\f27_ARp26_',num2str(state_code(1)),'.csv'],'Delimiter',',')
...
print(['..\plots\f27_ARp26_',UF],'-dpdf')
